...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!submitData.request_id) {
      return res.status(502).json({ error: 'No request_id: ' + JSON.stringify(submitData) });
    }

    const requestId = submitData.request_id;
    const base = `https://queue.fal.run/${endpoint}/requests/${requestId}`;

    // Poll until done — each slide is sequential so we have the full 58s
    ...
    return res.status(504).json({ error: 'fal timeout — try again' });
