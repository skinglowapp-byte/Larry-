export default async function handler(req, res) {
  ...
  if (req.method === 'POST' && req.body?.action === 'refresh_token') {
    ...
    try {
      ...
      if (data.error) {
        return res.status(400).json({ ok: false, code: data.error, message: data.error_description || data.error });
      }
      const expiresAt = Date.now() + ((data.expires_in || 86400) - 300) * 1000;
      return res.json({ ok: true, access_token: data.access_token, refresh_token: data.refresh_token, expires_at: expiresAt });
    } catch (e) {
      return res.status(500).json({ ok: false, code: 'refresh_failed', message: e.message });
    }
  }
  ...
  if (req.method === 'POST') {
    ...
    if (!access_token || !Array.isArray(images) || images.length === 0) {
      return res.status(400).json({ ok: false, code: 'bad_request', message: 'Missing access_token or images' });
    }
    ...
    try {
      ...
      if (!falKey) {
        return res.status(500).json({ ok: false, code: 'missing_fal_key', message: 'FAL key is not configured' });
      }
      const photoUrls = [];
      for (let i = 0; i < images.length; i++) {
        ...
        if (!falData.upload_url || !falData.file_url) {
          return res.status(500).json({ ok: false, code: 'fal_upload_init_failed', message: `FAL storage initiate failed for image ${i + 1}`, detail: falData });
        }
        ...
        if (!putRes.ok) {
          return res.status(500).json({ ok: false, code: 'fal_upload_failed', message: `FAL upload failed for image ${i + 1}`, detail: { status: putRes.status } });
        }
      ...
      return res.status(200).json({ ok: true, publish_id: initData?.data?.publish_id || null, post_mode: initPayload.post_mode });
    } catch (e) {
      return res.status(500).json({ ok: false, code: e?.payload?.code || 'server_error', message: e.message || 'Unexpected server error', log_id: e?.payload?.log_id || null, detail: e?.payload?.detail || null });
    }
  }
